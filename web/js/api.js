...
export const api = {
  ...
  renameBoard: (id, name) => req(`/api/boards/${id}`, json('PATCH', { name })),
  setBoardLanguage: (id, language) => req(`/api/boards/${id}/language`, json('PATCH', { language })),
  archiveBoard: (id) => req(`/api/boards/${id}/archive`, { method: 'POST' }),
  settings: () => req('/api/settings'),
  updateSettings: (language) => req('/api/settings', json('PATCH', { language })),
  ...
  version: (opts) => req(`/api/version${opts?.fresh ? '?fresh=1' : ''}`),
