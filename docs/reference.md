# Agentboard reference

The long version: data layout, every command, the runner, sessions,
backups. The short version is the [README](../README.md).
...
agentboard card edit task_a3f2 --labels wp,dns --context-refs acme/webshop.md

agentboard ctx list [pad]
agentboard ctx show acme/_client.md
cat file.md | agentboard ctx write acme/_client.md --content - --card ops_b71c --as agent
agentboard secret set ssh_acme_web03   # hidden prompt; or: echo "..." | agentboard secret set naam
agentboard secret get ssh_acme_web03
```
...
3. Every write to `context/` is exactly one git commit (card id in the
   message, e.g. `ctx: add site webshop (ops_b71c)`) plus one
...
the value lives in `secrets.env` (`ssh_acme_web03` → `SSH_ACME_WEB03`).
